...
import { secoesAtivos } from "../utils/CadastroAtivosEntradaTexto";
import { cadastrarAtivo } from "../servicos/AtivosServico";

export default function CriarAtivos({navigation}) {
  ...
  async function cadastrar() {
    const resultado = await cadastrarAtivo({
      id: 10,
  ...
  return (
    <ScrollView flex={1} p={5}>  
        <Box>
            {
            secoesAtivos[0]?.entradaTexto?.map(entrada => {
                return (
                <EntradaTexto 
                    label={entrada.label}
